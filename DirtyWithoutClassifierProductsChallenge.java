...
public class DirtyWithoutClassifierProductsChallenge {
	Random rand = new Random(1991);
	String folder = "ProductsChallenge";
	...
	ArrayList<String> blockRem = new ArrayList<String>();
        PrintStream feedbackQuality;// = new PrintStream("feedbackquality.txt");	
	...
	public double Weightedjaccard (int u, int v){
		...
		for(String a:l1){
			if(!blockWeight.containsKey(a))
				continue;
			 if (u==4304 && v==4306)
				System.out.println(a+" "+blockWeight.get(a));
			if(l2.contains(a)){
				inter+=(blockWeight.get(a));//*Math.min(s2.get(a), s1.get(a)));
				union+=(blockWeight.get(a));//*Math.max(s2.get(a), s1.get(a)));
			}else
	...
	if(prev_edge_prob.keySet().size() == edge_prob.keySet().size())
		...
		if(prev_edge_prob.containsKey(p) || prev_edge_prob.containsKey(p1))
         ...
         known_output.close();
         String[] args1 = new String[] {"/bin/bash", "-c", "cd ~/javaCode/src/CarsCombined; python learner.py > o1; cd .."};
 ...
 *                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         */ 
				...
				if(nr+ng>0){
					...
					for(int a: sortedMap.keySet()) {
						...
						if(currclust==0) {
							...
							entropy += (currclust*1.0/lst.size()) *Math.log(currclust*1.0/lst.size());
							numclust++;
						}
						else
							currclust--;
					...
					if(fulldummy ==1)
						...
						//if(lst.size()>1)
						blockWeight.put(s, (ng*1.0/(ng+nr))*Math.exp(entropy) );//(maxsize*1.0/lst.size())//clustsize.keySet().size()		
					}
				...
				*/
				...
				HashMap<Integer,Integer> clustsize = new HashMap<Integer,Integer>();
				{
					...
					if(nr+ng>0){
						...
						bw.println(i+" "+s+" "+lst.size()+" "+gr*1.0/(gr+rd)+" "+Math.exp(entropy)+" "+normal+" "+Math.log(recordList.keySet().size()*1.0/lst.size())*1.0/Math.log(recordList.keySet().size())+" "+blockWeight.get(s));
	...
	public void GetExpected(boolean first) throws FileNotFoundException{
		...
		//
		int gg=0;
        PrintStream outfile = new PrintStream("chevyweight.txt");	
			...
			//		continue;	
				...
				ArrayList<Integer> lst = BlockTreeMap.get(level).get(s);
				//if (lst.size()>1)
				//outfile.println(s+" "+level+" "+s.split("!",0)+" "+lst.size());

				System.out.println(s+" "+SimilarityList.size()+" "+lst.size());
				...
				for(int i1=0;i1<lst.size();i1++){
					...
					for(int i2=i1+1;i2<lst.size();i2++){
						...
						if  (goldMap.get(u).equals(goldMap.get(v)))
								curr_gg++;
						if(processed.containsKey(t) || processed.containsKey(t1))//|| folderMap.get(u)==folderMap.get(v))
							continue;
						else {
													outfile.println(u+" "+v+" "+goldMap.get(u).equals(goldMap.get(v)));
			...
			int tot=(int)(lst.size()*(lst.size()-1)/2);
			if(tot>0&&curr_gg>=tot)
				feedbackQuality.println(curr_gg+" "+s+" "+lst.size()+" "+goldMap.get(lst.get(0)));
			}
			processed.clear();
			//System.out.println("similarity list size is "+SimilarityList.size());
	        	//System.out.println(queried_edge_map.keySet().size()+"size is ");
			System.out.println("feedback quality is "+gg);
			feedbackQuality.println(gg);
			if(first)
				return;        	
			//if(!foundnew)
	        	//	return;
...
//			System.out.println(recordList1.get(p.x)+" "+s1);
	...
	public int mergeBlocks() {
		...
		for(Integer rec_id : recordList.keySet()) {
			...
			String tmp = partitions[0];//+" "+partitions[2];
			...
			for(;i<rec.length;i++) {
				...
				while (j<rec.length) {
					...
					if(token.equals("bags!!"))
						System.out.println(i+" "+j+" "+rec+" dsadsajdksajlkdjsalk"+" "+tmp);
...
//		HashMap<String,Double> ruleBlockWeight = new HashMap<String,Double>();
		...
		for(int i=0;i<BlockTreeMap.keySet().size();i++) {
			...
			for(String s:level.keySet()) {
				...
				System.out.println(s+" "+blockWeight.get(s)+" "+level.get(s).size());
				/*else {
		...
		while(true){
			...
			max=maxnode;
			System.out.println("max_benefit"+max+" "+goldMap.get(max)+" "+max_benefit);
			...
			for(double[] comp : comp_list){
				...
				for(int q : current){
					...
					System.out.println(q+" "+max+" "+output[1]+" "+output[0]+" "+edge_prob.get(t));
					System.out.println(goldMap.get(q)+" "+goldMap.get(max));
					if(!queried_edge_map.containsKey(t)){
						queries++;
			...
			if(processed_nodes.size()%100==0){
				...
				{
					for(Integer rec_id : recordList.keySet()) {
						int gt_id=goldMap.get(rec_id);
						for(Integer rec_id2 : recordList.keySet()) {
							pair t = new pair(rec_id,rec_id2);
							if (rec_id2>rec_id && gt_id==goldMap.get(rec_id2) && !edge_prob.containsKey(t)){
								System.out.println(rec_id+" "+rec_id2+" "+recordList.get(rec_id)+" "+recordList.get(rec_id2));
							}
						}
					}
			...
			//	if(processed_nodes.size()==100)
				if(pipelinetype.equals("pM3loop") || pipelinetype.equals("ploop") || pipelinetype.equals("iM3loop")|| pipelinetype.equals("nM3loop"))
					if (updated || pipelinetype.equals("iM3loop")|| pipelinetype.equals("nM3loop")){
						GetExpected(false);
	...
	public static void main(String[] args) throws Exception{
		...
		DirtyWithoutClassifierProductsChallenge FF = new DirtyWithoutClassifierProductsChallenge();
		...
		while(scanner.hasNextLine()){
			...
			HashMap<String,Integer> recordMap = new HashMap<String,Integer>();
			int recordId = Integer.parseInt(attributeList[0].split(" ",0)[0].trim().replace(".jpg", ""));
			
			//if(!largest.contains(recordId))
				String[] tokenList = attributeList[0].split(" ",0);
		...
		String folderFile     = FF.folder+"/folder_map.txt";

		Scanner folderScanner = new Scanner(new File(folderFile));
		while(folderScanner.hasNextLine()){
			line= folderScanner.nextLine();
			//System.out.println(line);
			String[] foldertokens = line.split(" ",0);
			int u = Integer.parseInt(foldertokens[0].replace(".jpg", ""));
			id = Integer.parseInt(foldertokens[1]);
			FF.folderMap.put(u, id);
			//System.out.println(u+" "+id+" "+FF.goldMap.get(5));
		}
		 id = FF.recordList.keySet().size();
		for(int i:FF.recordList.keySet()){
			FF.N++;
			if(FF.folderMap.containsKey(i))
				continue;
			else{
				FF.goldMap.put(i, id);
				id++;
		...
		FF.feedbackQuality= new PrintStream("feedbackquality.txt");
		FF.processBlocks();
