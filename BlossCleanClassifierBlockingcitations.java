...
public class BlossCleanClassifierBlockingcitations {
	Random rand = new Random(1991);
	static Random generator = new Random(1992);

	String folder = "citations";//"songs";
	...
	double g_to_r=0.0;
	double r_to_g=0.0;
	boolean useClassifier=false;//true;//false;//true;
	int mergedBlocks=-1;
...
public pair get_best_pair(HashMap<String,Boolean> covered_features, ArrayList<pair> level, HashMap<pair,ArrayList<Integer>> blocking_features){
		...
		
		{
		 ...
		 String falcon_data = folder+"/falcondata.txt";
			...
			if(!(recordList1.containsKey(p.x) && recordList2.containsKey(p.y)))
                                continue;
		...
		String[] args1 = new String[] {"/bin/bash", "-c", "cd ~/javaCode/src/citations; python learnerbloss.py > b1; cd .."};
                try{
			...
			for(Integer rec_id : recordList.keySet()) {
				...
				for(;i<rec.length;i++) {
					...
					while (j<=rec.length) {
						...
						if(jj==2 && !BlockTreeMap1.get(level).containsKey(token))
							break;
			...
			for(int i=0;i<depth;i++) {
				...
				for(String s:level_map.keySet()) {
					...
					//if(level_map.get(s).size()>10 || tokenLst.length==1)
					{
						...
						//	continue;	
						if (tokenLst.length>1) {
		//				if(tmpBlockTreeMap.get(tokenLst.length-2).get(String.join("!", Arrays.copyOfRange(tokenLst, 0, tokenLst.length-1))).size()==1)				
		//							continue;
		//				if(tmpBlockTreeMap.get(tokenLst.length-2).get(String.join("!", Arrays.copyOfRange(tokenLst, 1, tokenLst.length))).size()==1)				
		//					continue;
							ArrayList<String> parent_lst = new ArrayList<String>();
							...
							for (String parent: parent_lst){
								...
								int psize=tmpBlockTreeMap.get(parent_level).get(parent).size();
								if (psize==1){
									noneed=true;
									break;
								}
...
//			System.out.println(s1[i]+" "+i+" "+s2[i]);
			...
			//System.out.println(p.x+" "+p.y+" "+recordList1.get(p.x)+" "+s1);
			HashMap<String,Double> CurrFeatMap;
			if (i==1 || i==2) {
	...
	public ArrayList<HashMap<String,Double[]>> TrainClassifier(HashMap<pair,Boolean> training) throws Exception {
	 ...
	 public boolean  TrainClassifierPython(ArrayList<double[]> SimList, int start,int num, boolean newsamples) throws Exception{
		...
		String falcon_data = folder+"/falcondata.txt";
         ...
         known_output.close();
         String[] args1 = new String[] {"/bin/bash", "-c", "cd ~/javaCode/src/citations; python learner.py > o1; cd .."};
         Process proc = new ProcessBuilder(args1).start();
          proc.waitFor();
...
*/
		...
		boolean ooutput = TrainClassifierPython(SimilarityList, 0, feedback_val, true);
		int curr_iter = 0;
		...
		for(pair p : goldMap.keySet()) {
			...
			*/
				if(!edge_prob.containsKey(p)){
					...
					pr.println(inverted_list1.get(p.x)+" "+inverted_list2.get(p.y));
	...
	public static void main(String[] args) throws Exception{
		...
		BlossCleanClassifierBlockingcitations FF = new BlossCleanClassifierBlockingcitations();
