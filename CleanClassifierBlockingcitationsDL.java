...
public class CleanClassifierBlockingcitationsDL {
	...
	double g_to_r=0.0;
	double r_to_g=0.0;
	boolean useClassifier=true;//true;//false;//true;
	int mergedBlocks=-1;
...
//			System.out.println(s1[i]+" "+i+" "+s2[i]);
	...
	public ArrayList<HashMap<String,Double[]>> TrainClassifier(HashMap<pair,Boolean> training) throws Exception {
	 ...
	 public boolean  TrainClassifierPython(ArrayList<double[]> SimList, int start,int num, boolean newsamples) throws Exception{
		...
		String falcon_data = folder+"/falcondata.txt";
         ...
         for(int i=start;;i++) {
		   ...
		   if (i<=start+num){
                  ...
                  */
			if(goldMap.containsKey(p))
                                classifier_output.print((p.x)+" "+(p.y)+" 1.0\n");
                        else
                                classifier_output.print((p.x)+" "+(p.y)+" 0.0\n");
		}else{/*
                          for (String s: feat.keySet()){
                                  if( s.equals("Class"))
                                          continue;
			...
			*/
			if(goldMap.containsKey(p))
                                unseen_output.print((p.x)+" "+(p.y)+" 1.0\n");
                        else
                                unseen_output.print((p.x)+" "+(p.y)+" 0.0\n");
		...
		if(goldMap.containsKey(p))
                                known_output.print((p.x)+" "+(p.y)+" 1.0\n");
                        else
                                known_output.print((p.x)+" "+(p.y)+" 0.0\n");
...
*/
	...
	public static void main(String[] args) throws Exception{
		...
		CleanClassifierBlockingcitationsDL FF = new CleanClassifierBlockingcitationsDL();
