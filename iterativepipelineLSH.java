...
public class iterativepipelineLSH {
	...
	public void GetExpected(ArrayList<ArrayList<Integer>> Newblocks ) throws FileNotFoundException{
		...
		for(int i=0;i<Newblocks.size();i++) {
			ArrayList<Integer> lst = Newblocks.get(i);
			double gr=0,rd=0;
			for(int i1=0;i1<lst.size();i1++){
				int u = lst.get(i1);
				for(int i2=i1+1;i2<lst.size();i2++){
	...
	public ArrayList<Double> RandomVector(int n){
		...
		for(int i=0;i<n;i++) {
			tmp.set(i, tmp.get(i)*1.0/Math.sqrt(sum));
		}
		return tmp;
		
	}
	public double GetComponent(ArrayList<Double> vec, HashMap<String,Integer> record) {
		double sum=0.0;
		for(String a:record.keySet()) {
			sum += blockWeight.get(blockIndex.get(a))*vec.get(blockIndex.get(a)) * record.get(a);
		}
		return sum;
	}
	public double calculatedist(ArrayList<Double> v1, ArrayList<Double> v2) {
		double dist = 0.0;
	...
	public ArrayList<Double>  addvec(ArrayList<Double> a,ArrayList<Double> b){
		ArrayList<Double> tmp = new ArrayList<Double>();
	...
	public ArrayList<ArrayList<Integer>> Kmeans(ArrayList<ArrayList<Double>> recordEmbedding , int c){
		...
		for(int i=0; i<c;) {
			int val = rand.nextInt(recordList.keySet().size());
			if(selected.contains(val))
				continue;
			seeds.add(recordEmbedding.get(val));
			i++;
		}
		assignment.clear();
		for(int iter= 0;iter<100;iter++) {
			for(int i=0; i<recordEmbedding.size();i++) {
				double mindist =100;
				for(int j=0;j <seeds.size();j++) {
					double dist = calculatedist(seeds.get(j),recordEmbedding.get(i));
					if(dist < mindist) {
						mindist = dist;
						assignment.put(i, j);
					}
			...
			ArrayList<ArrayList<Double>> newseeds = new ArrayList<ArrayList<Double>>();
			...
			for(int j=0;j<recordEmbedding.size();j++) {
				
				ArrayList<Double> tmp = newseeds.get(assignment.get(j));
				tmp = addvec(tmp,recordEmbedding.get(j));
				Size.set(assignment.get(j), Size.get(assignment.get(j))+1);
				newseeds.set(assignment.get(j), tmp);
			}
			
			for(int i=0;i<newseeds.size();i++) {
				ArrayList<Double> tmp = newseeds.get(i);
				for(int j=0;j<tmp.size();j++) {
					tmp.set(j, tmp.get(j)*1.0/Size.get(i));
				}
				seeds.set(i, tmp);
			}
		...
		for(int i=0;i<recordEmbedding.size();i++) {
			ArrayList<Integer> tmp = clusters.get(assignment.get(i));
			tmp.add(i);
			clusters.set(assignment.get(i), tmp );
		}
	...
	public void processBlocks() throws FileNotFoundException{
		...
		ArrayList<ArrayList<Double>> recordEmbedding = new ArrayList<ArrayList<Double>>(); 
		int pmap=20;
		for(int m =0;m<pmap;m++) {
			ArrayList<Double> u  = RandomVector(blockSize.size());
			for(int i=0;i<recordList.keySet().size();i++) {
				if(m==0) {
					ArrayList<Double> tmp = new ArrayList<Double>();
					tmp.add(GetComponent(u,recordList.get(i)));
					recordEmbedding.add(tmp);
				}else {
					ArrayList<Double> tmp = recordEmbedding.get(i) ;
					tmp.add(GetComponent(u,recordList.get(i)));
					recordEmbedding.set(i, tmp);
				}	
			}
		}
		...
		ArrayList<ArrayList<Integer>> Newblocks = Kmeans(recordEmbedding , 10);
		for(int i=0;i<Newblocks.size();i++) {
			System.out.println("New blocks are "+i+" "+Newblocks.get(i).size());
			ArrayList<Integer> block = Newblocks.get(i);
			int gr=0, rd=0;
			...
			if(processed_nodes.size()%100==0){
				...
				// if(processed_nodes.size()>=1000)
				 ...
				 for(int m =0;m<pmap;m++) {
					...
					Newblocks = Kmeans(recordEmbedding , 10);
					for(int i=0;i<Newblocks.size();i++) {
						System.out.println("New blocks are "+i+" "+Newblocks.get(i).size());
						ArrayList<Integer> block = Newblocks.get(i);
						int gr=0, rd=0;
	...
	public static void main(String[] args) throws Exception{
    		...
    		iterativepipelineLSH FF = new iterativepipelineLSH();
