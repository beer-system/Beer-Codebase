...
public class CoraDirtyWithoutClassifierCorrelationClustering {
	...
	double g_to_r=0.0;
	double r_to_g=0.0;
	boolean useClassifier=false;//true;//false;//true;//false;
	int mergedBlocks=-1;
...
*/
	...
	*/
	...
	public double cc_entropy(ArrayList<Integer> lst){
		double delta=1.0/44;//2.0/21;
                ...
                for(int i1 = 0;i1<lst.size();i1++){
	                   ...
	                   for(int i2=i1+1;i2<lst.size();i2++){
                            ...
                            /*if(edge_prob.containsKey(t)) {
                                   if(edge_prob.get(t)>0.5){
						
					
						ArrayList<Integer> t1 = adj_list.get(i1);
						t1.add(i2);
						adj_list.put(i1,t1);
						ArrayList<Integer> t2 = adj_list.get(i1);
						t2.add(i2);
						adj_list.put(i1,t2);
					}
...
*/
						...
						t1.add(i2);
						adj_list.put(i1,t1);
						ArrayList<Integer> t2 = adj_list.get(i2);
						t2.add(i1);
						adj_list.put(i2,t2);
				}else{
						...
						//			p=p*0.5;
									if(p>0.5){
									//	System.out.println(p+"this is there");	
										ArrayList<Integer> t1 = adj_list.get(i1);
                                  						              t1.add(i2);
                                   						             adj_list.put(i1,t1);
                                    						            ArrayList<Integer> t2 = adj_list.get(i2);
...
//
		...
		int index=(int)(Math.random()*(tmp_lst.size()-1));	
		index=tmp_lst.get(index);
		//while(covered.contains(index))
		//	index=(int)(Math.random()*(lst.size()-1));


		ArrayList<Integer> Nu=adj_list.get(index);
		ArrayList<Integer> NewNu=new ArrayList<Integer>();//adj_list.get(index);

		for(int i=0;i<(Nu.size());i++){
			if(covered.contains(Nu.get(i)) )
				continue;
			else
				NewNu.add(Nu.get(i));
		...
		Nu=NewNu;
		ArrayList<Integer> VNu = new ArrayList<Integer>();
		...
		System.out.println("entropy"+NewNu.size()+" "+lst.size()+" "+entropy);
		while(true){
			int i=0;
			boolean changed=false;
			while (i<Nu.size()){
				int v=Nu.get(i);
				ArrayList<Integer> Nv=adj_list.get(v);
				//Sets.intersection(Sets.newHashSet(Nu), Sets.newHashSet(Nv));
				int intersection=get_intersection(Nu,Nv,covered);
				int intersection2=get_intersection(Nv,VNu,covered);
				if(intersection <= (int)((1-3*delta)*Nu.size())){
					Nu.remove(i);
					VNu.add(i);
					changed=true;
					break;
				}else if(intersection2 > (int)((3*delta)*Nu.size())){
                                        Nu.remove(i);
					VNu.add(i);
                                        changed=true;
                                        break;
                                }
	//			System.out.println(changed+" "+Nu.size());
		...
		ArrayList<Integer> new_added=new ArrayList<Integer>();

		for(int v:VNu){
				 ArrayList<Integer> Nv=adj_list.get(v);
				int intersection=get_intersection(Nu,Nv,covered);
                                int intersection2=get_intersection(Nv,VNu,covered);
				 if(intersection > (int)((1-7*delta)*Nu.size()) && intersection2 <= (int)((7*delta)*Nu.size())){
                                        new_added.add(v);
                                   //     changed=true;
                                }//else if(intersection2 > (int)((3*delta)*Nu.size())){
		...
		Nu.add(index);
		covered.addAll(Nu);
		entropy+= (Nu.size()*1.0/lst.size())*Math.log(Nu.size()*1.0/lst.size());


		System.out.println(Nu.size()+" "+lst.size()+" "+covered.size()+" hhhhh"+Nu.size()*1.0/lst.size()+" "+entropy);
	}

		if(entropy!=0)
			System.out.println(entropy+" "+lst.size());//Nu.size()*1.0/lst.size());
		return entropy;
	}
	public int get_intersection(ArrayList<Integer> Nu,ArrayList<Integer>Nv,ArrayList<Integer> covered){
		int inter=0;
		for (int a:Nu){
			...
			HashMap<Integer,Integer> clustsize = new HashMap<Integer,Integer>();
			{
				...
				if(nr+ng>0){
					...
					if(fulldummy ==1)
						...
						//if(lst.size()>1)
						//blockWeight.put(s, (ng*1.0/(ng+nr))*(1+entropy) );//(maxsize*1.0/lst.size())//clustsize.keySet().size()		
	...
	public boolean UpdateWeight() throws FileNotFoundException{
		...
		for (int i=0;i<BlockTreeMap.keySet().size();i++) {
			...
			for(String s:level.keySet()) {
				...
				double ccentropy = 0.0;//cc_entropy(lst);
				...
				*/
				...
				HashMap<Integer,Integer> clustsize = new HashMap<Integer,Integer>();
				{
					...
					if(nr+ng>0){
						...
						System.out.println(ng*1.0/(ng+nr));					
						//blockWeight.put(blockIndex.get(s), Math.pow((ng*1.0/(ng+nr)),1)*(maxsize*1.0/lst.size())*(1.0/clustsize.keySet().size()));//(maxsize*1.0/lst.size())
						int fulldummy = 6;
						//1 = full dummy
						//0 = Full converged
						...
						if(fulldummy ==1)
							...
							//if(lst.size()>1)
							blockWeight.put(s, (ng*1.0/(ng+nr))*Math.exp(ccentropy) );//(maxsize*1.0/lst.size())//clustsize.keySet().size()		
							//blockWeight.put(s, (ng*1.0/(ng+nr))*Math.exp(ccentropy) );//(maxsize*1.0/lst.size())//clustsize.keySet().size()		
						}
						//bw.println(i+" "+s+" "+lst.size()+" "+gr*1.0/(gr+rd)+" "+weighted+" "+normal+" "+Math.log(recordList.keySet().size()*1.0/lst.size())*1.0/Math.log(recordList.keySet().size())+" "+blockWeight.get(blockIndex.get(s)));
...
//		HashMap<String,Double> ruleBlockWeight = new HashMap<String,Double>();
	...
	public static void main(String[] args) throws Exception{
		...
		CoraDirtyWithoutClassifierCorrelationClustering FF = new CoraDirtyWithoutClassifierCorrelationClustering();
