...
public class OurPipelineCars {
	Random rand = new Random(1991);
	String folder = "carsStan";
	...
	*/
	...
	public void UpdateWeight() throws FileNotFoundException{
		...
		for(int i=0;i<blockSize.size();i++) {
			...
			if (i>5185 && !s.equals("chevrolet")) {
				blockWeight.put(blockIndex.get(s),0.0);//weightlast*Math.log(recordList.keySet().size()*1.0/lst.size())*1.0/Math.log(recordList.keySet().size())/(Math.log(recordList.keySet().size()*1.0/blockList.get(blockIndex.get(slast)).size())*1.0/Math.log(recordList.keySet().size())));
	...
	public void processBlocks() throws Exception{
		...
		for( int i=0;i<blockSize.size();i++){
			...
			int g=0,r=0,gp=0,rp=0,tot=0;
			HashMap<Integer,Integer> gtmap=new HashMap<Integer,Integer>();
			for(int a:lst) {
				if(gtmap.containsKey(goldMap.get(a))) {
					int v=gtmap.get(goldMap.get(a));
					gtmap.put(goldMap.get(a),v+1);
					
					
				}else
					gtmap.put(goldMap.get(a),1);
					
				for(int b:lst) {
					if(a>b) {
						if(goldMap.get(a).equals(goldMap.get(b))) {
							if(goldMap.get(a)==118) {
								gp += Weightedjaccard(GetDict(recordList.get(a)),GetDict(recordList.get(b)));
								tot++;
							}
							...
							rp += Weightedjaccard(GetDict(recordList.get(a)),GetDict(recordList.get(b)));
							r++;
						}
			...
			System.out.println(g+" "+r+" "+g*1.0/(g+r)+" "+gp*1.0/(tot)+" "+tot+" "+rp*1.0/(r));
		...
		*/
		...
		//chevrolet, sedan, aston,
		//When to merge: when two blocks are highly correlated then it is just redundant information
		//When to split?
		/*
		for( int i=0;i<blockSize.size();i++){
			blockId = (int) blockSize.get(i)[0];
			...
			ArrayList<Integer> lst = blockList.get(blockIndex.get(s));
			System.out.println(s+" "+lst.size());
			String[] removetokens = s.split("!",0);
			//if(!s.equals("chevrolet"))
			//	continue;
			if(i>=mergedBlocks && mergedBlocks>0)
				break;
			if(lst.size()<10)
				continue;
			if (lst.size()>1000)
			...
			ArrayList<Integer[]> blockdist = new ArrayList<Integer[]>();
			
			for(int a:lst) {
				for(String s1: GetDict(recordList.get(a)).keySet()) {
					if(s1.equals(s))
						continue;
					if(blockjoin.containsKey(s)) {
						if(s.equals("cargo"))
							System.out.println(a+" "+s1+" "+blockjoin.get(s)+"FDSFDFDSDS");
						boolean found=false;
						for(String a1:blockjoin.get(s)) {
							if(a1.equals(s1) ) {
								found=true;
								break;
							}
							for(String b1:a1.split("!",0)) {
								if(s1.contains(b1))
									found=true;
								else {
									found=false;
					...
					boolean found=false;
					for(String a1:removetokens) {
						//System.out.println(a1+" "+(removetokens.length));
						if(s1.contains(a1))
							found=true;
					}
					//Added for now until i handle everything correctly
					
					if(found)
						continue;
					//if(s.equals("srt-4"))
					//	System.out.println(recordList.get(a)+" ");
					if (blockMap.containsKey(s1)) {
						int v=blockMap.get(s1);
						blockMap.put(s1, v+1);
					}else {
						blockMap.put(s1, 1);
					}
			...
			for(String s1:blockMap.keySet()) {
				Integer[] e = {blockIndex.get(s1),blockMap.get(s1)};
				blockdist.add(e);
				//System.out.println(s1+" "+blockdist.get(s1)*1.0/(lst.size()));
			}
				
			Collections.sort(blockdist, new Comparator<Integer[]>() {
				public int compare(Integer[] o1, Integer[] o2) {
					Integer s1 = o1[1]; Integer s2 = o2[1];
					if (s1 != s2)
						return (s1 > s2 ? -1 : 1);
					else
						return 0;
			...
			for(Integer[] e:blockdist) {
				if(e[1]<10)
					break;
				if(e[1]*1.0/lst.size()  >= 0.7) {//5*blockList.get(e[0]).size()*1.0/recordList.size()) {
					System.out.println(s+"!"+Index2Str.get(e[0])+" "+Index2Str.get(e[0])+" "+e[1]*1.0/(lst.size())+" "+e[1]+" "+lst.size()+" "+blockList.get(e[0]).size()*1.0);
					System.out.println("reduccing block size"+i);
					if(blockjoin.containsKey("fellows-corvette"))
						System.out.println(blockjoin.get("fellows-corvette").size()+" "+blockjoin.get("corvette").get(0));
					ArrayList<Integer> lst1 = blockList.get(e[0]);
					//Consider a block of intersection
					if(mergedBlocks==-1)
						mergedBlocks = blockList.size();
					ArrayList<Integer> tmplst = GetIntersection(lst,lst1);
					//for (Integer a : tmplst) {
					//    lst.remove(a);
				//	}
					//for (Integer a : tmplst) {
					//    lst1.remove(a);
					...
					if(lst1.size()==0) {
						blockRem.add(Index2Str.get(e[0]));
					}
					
					
						
					if(blockjoin.containsKey(s)) {
						ArrayList<String> l = blockjoin.get(s);
						l.add(Index2Str.get(e[0]));
						blockjoin.put(s, l);
					}else {
					...
					if(blockjoin.containsKey(Index2Str.get(e[0]))) {
						ArrayList<String> l = blockjoin.get(Index2Str.get(e[0]));
						l.add(s);
						blockjoin.put(Index2Str.get(e[0]), l);
					}else {
						ArrayList<String> l = new ArrayList<String>();
						l.add(s);
						blockjoin.put(Index2Str.get(e[0]), l);
					...
					//blockList.set(blockIndex.get(s),lst);
					//blockList.set(blockIndex.get(Index2Str.get(e[0])),lst1);
					blockList.add(tmplst);
					System.out.println(lst1.size()+" "+blockList.size()+" "+s+" "+e[0]+" "+tmplst.size());
					//Get intersection block
					
					blockIndex.put(s+"!"+Index2Str.get(e[0]),blockList.size()-1);
					blockIndex.put(Index2Str.get(e[0])+"!"+s,blockList.size()-1);
					
					Index2Str.put(blockList.size()-1, s+"!"+Index2Str.get(e[0]));
					double[] tmp = {blockList.size()-1,e[1]};
					blockSize.add(tmp);
					i--;
					changed=true;
					if(Index2Str.get(e[0]).equals("rt!dodge")) {
						System.out.println(lst.size()+" "+lst1.size()+" "+tmplst.size()+" "+blockjoin.get("rt!dodge")+" "+blockjoin.get("srt-4"));
				...
				//out.println(Index2Str.get(e[0])+" "+e[1]*1.0/(lst.size()));
				
			}
			
			
			//out.close();
			System.out.println("going back"+i+" "+lst.size());
			if(changed)
				continue;
			i11++;
		}
		...
		*/
		...
		/*for(int j=mergedBlocks;j<blockList.size();j++) {
			ArrayList<Integer> l1 = blockList.get(j);
			int g=0,r=0;
			for(int a:l1) {
				for(int b:l1) {
					if(a>b) {
			...
			System.out.println(Index2Str.get(j)+" "+g+" "+r+" "+l1.size());
		...
		for(int j=0;j<blockList.size();j++) {
			double[] tmp = {j,blockList.get(j).size()};
			blockSize.add(tmp);
		}*/
		...
		*/
		...
		for(int i=0;i<blockSize.size();i++){
			...
			if(pipelinetype.equals("pM3loop") || pipelinetype.equals("ploop") || pipelinetype.equals("iM3loop")||pipelinetype.equals("iNoloop")) {
				if(i>5191) {
					String s = Index2Str.get(blockId);
					String[] tokenList = s.split("!",0);
					//blockIndex.put(s+"!"+Index2Str.get(e[0]),blockList.size()-1);
					//blockIndex.put(Index2Str.get(e[0])+"!"+s,blockList.size()-1);
					//Index2Str.put(blockList.size()-1, s+"!"+Index2Str.get(e[0]));
					double val=0;
					for(String a:tokenList) {
						val+= blockWeight.get(blockIndex.get(a));
					}
					if(val<Math.log(recordList.keySet().size()*1.0/block.size())*1.0/Math.log(recordList.keySet().size())) {
						blockWeight.put(blockId, Math.log(recordList.keySet().size()*1.0/block.size())*1.0/Math.log(recordList.keySet().size()));
						System.out.println("*******************************FOUND THIS");
						return;
					}else {
						blockWeight.put(blockId, -val+Math.log(recordList.keySet().size()*1.0/block.size())*1.0/Math.log(recordList.keySet().size()));
	...
	public void readWeight() throws FileNotFoundException {
		...
		while(scanner.hasNextLine()){
			...
			if(Double.valueOf(attributeList[3])<0.2)
				blockWeight.put(blockIndex.get(attributeList[1]),Double.valueOf(attributeList[7]));//weightlast*Math.log(recordList.keySet().size()*1.0/lst.size())*1.0/Math.log(recordList.keySet().size())/(Math.log(recordList.keySet().size()*1.0/blockList.get(blockIndex.get(slast)).size())*1.0/Math.log(recordList.keySet().size())));
			else
				blockWeight.put(blockIndex.get(attributeList[1]),(1-0.004326077071154559)*Double.valueOf(attributeList[6]));//weightlast*Math.log(recordList.keySet().size()*1.0/lst.size())*1.0/Math.log(recordList.keySet().size())/(Math.log(recordList.keySet().size()*1.0/blockList.get(blockIndex.get(slast)).size())*1.0/Math.log(recordList.keySet().size())));
	...
	public static void main(String[] args) throws Exception{
		...
		OurPipelineCars FF = new OurPipelineCars();
