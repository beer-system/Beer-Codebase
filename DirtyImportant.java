...
public class DirtyImportant {
	...
	double g_to_r=0.1;
	double r_to_g=0.1;
	boolean useClassifier=true;//false;
	int mergedBlocks=-1;
	 ...
	 public boolean  TrainClassifierPython(ArrayList<double[]> SimList, int start,int num, boolean newsamples) throws Exception{
         ...
         for(int i=1;i<=recordList.size();i++) {
        	...
        	for(int j=i+1;j<recordList.size();j++)  {
                  ...
                  pair p = new pair(i,j);
                  pair p1 = new pair(j,i);//(int)curr[2],(int)curr[1]);
		...
		for(int a:NewNodes){
			...
			*/
			...
			for(String s: inverted_list.get(a)){
				int total=numGreen.get(s)+numRed.get(s);
				int level = s.split("!",0).length-1;
                                ArrayList<Integer> lst = BlockTreeMap.get(level).get(s);
				if(total>5000  || total >= lst.size()*0.5)
					continue;
				else
					updateneeded=true;
				if(updateneeded)
...
//		HashMap<String,Double> ruleBlockWeight = new HashMap<String,Double>();
		...
		System.out.println("entering python");

	 	boolean ooutput =  TrainClassifierPython( SimilarityList,0,100000000,true);//int num, boolean newsamples) throws Exception{
		...
		while(true){
			...
			if(processed_nodes.size()==N)
				break;
			if(processed_nodes.size()%1000==0){
	...
	public static void main(String[] args) throws Exception{
		...
		DirtyImportant FF = new DirtyImportant();
