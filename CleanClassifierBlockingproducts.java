...
public class CleanClassifierBlockingproducts {
	Random rand = new Random(1991);
	static Random generator = new Random(1992);

	String folder = "products";//"citations";//"songs";
	...
	double g_to_r=0.0;
	double r_to_g=0.0;
	boolean useClassifier=false;//true;//true;
	int mergedBlocks=-1;
	...
	public boolean[] query_edge_prob(HashMap<pair,Boolean>  oracle, HashMap<pair, Integer> gt, int u, int v, double r_to_g, double g_to_r) throws Exception{
		...
		if(useClassifier) {
			if(inferred_edges.containsKey(tmp) )
				ret[1]=true;
	...
	*/
	...
	public void GetExpected() throws FileNotFoundException{
		...
		for (String key : result.keySet()) { 
			...
			if(!BlockTreeMap2.get(level).containsKey(s))
				continue;
			...
			//if(iter%10000==0)
			//if(level>0)
				System.out.println(iter+" "+s+" "+lst2.size()+" "+lst1.size()+" "+SimilarityList.size());//+"k"+BlockTreeMap1.get(level-1).containsKey(s));//+" "+lst1.size());//+" "+lst1.size()+" "+blockWeight1.get(s));
			iter++;

			if(s.equals("sony"))
				System.out.println(s+" "+lst1.size()+" "+lst2.size()+" "+level);
			if (SimilarityList.size()>5000000) {
				System.out.println(iter+" "+s+" "+lst1.size()+" "+SimilarityList.size()+" "+blockWeight1.get(s));//+"k"+BlockTreeMap1.get(level-1).containsKey(s));//+" "+lst1.size());//+" "+lst1.size()+" "+blockWeight1.get(s));
				break;
			}
			/*
...
*/		//SimilarityList.clear();	
					...
					//if(level_map.get(s).size()>10 || tokenLst.length==1)
					{
						...
						if (tokenLst.length>1) {
							...
							for (String parent: parent_lst){
								...
								parent_p *= p1;
								//if(jj==2){
									//if (BlockTreeMap1.get(i).containsKey(parent)){
									//double score1 = recordList1.keySet().size()*1.0/BlockTreeMap1.get(i).get(parent).size();
                                                        		double score2 = 1.0/p1;//recordList2.keySet().size()*1.0/BlockTreeMap2.get(i).get(s).size();
                                                        		//blockWeight1.put(s, Math.log(score1*score2)*1.0/(totalpairs));
                                                        		parent_score*=Math.log(score2)*1.0/(tmpBlockTreeMap.get(parent_level).get(parent).size());
									//}
								//}
							...
							*/
							double curr_p = level_map.get(s).size()*1.0/recordList.size();
							double curr_score=1.0;
							//if (jj==2 && BlockTreeMap1.get(i).containsKey(s)){
								
							//	double score1 = recordList1.keySet().size()*1.0/BlockTreeMap1.get(i).get(s).size();
                                                                        double score2 = 1.0/curr_p;//recordList2.keySet().size()*1.0/BlockTreeMap2.get(i).get(s).size();
								curr_score*=Math.log(score2)*1.0/(level_map.get(s).size());

							//}
							if (curr_p > parent_p  || curr_score >parent_score) {
	...
	public HashMap<String,Double>    Get_Numeric_features(String a,String b){
		HashMap<String,Double> featMap = new HashMap<String,Double>();
		double v1 = 0.0;//Double.valueOf(a);
		double v2 = 0.0;
		if (!b.equals(""))v2 = Double.valueOf(b);
		...
		if (v1==0.0)
		featMap.put("relative_diff", 0.0);//Math.abs((v1-v2)*1.0/v1));
		else featMap.put("relative_diff", Math.abs((v1-v2)*1.0/v1));
		//featMap.put("levenshtein", Levenshtein_sim (a,b) );
	...
	public HashMap<String,Double> Extractfeat(pair p, Boolean cl){
		...
		for (int i=1;i<7;i++) {
			if(s1.length<=i || s2.length<=i) {
				...
				CurrFeatMap  = Get_Numeric_features("","");
				//	CurrFeatMap  = Get_Single_Word_features("","");
				}
...
*/		int numClassifiers=10;
	...
	HashMap<Integer,Integer> blocking_edges = new HashMap<Integer,Integer>();
		...
		//final long startTime4 = System.currentTimeMillis();
                  ...
                  HashMap<String,Double> feat = new HashMap<String,Double>();
                  double[] curr = SimList.get(i);
		 //System.out.println(i+" "+curr[1]+" "+curr[2]);
                  pair p = new pair((int)curr[1],(int)curr[2]);
                  if(!edge_prob.containsKey(p))
                          continue;
                  if(goldMap.containsKey(p))
		...
		//	totalTime += endTime4-startTime4; 
         ...
         known_output.close();
         String[] args1 = new String[] {"/bin/bash", "-c", "cd ~/javaCode/src/products; python learner.py > o1; cd .."};
		...
		for(int i=0;i<BlockTreeMap1.keySet().size();i++) {
			...
			for(String s:level.keySet()) {
				//if (i==0) 
				{
					...
					System.out.println(s+" "+blockWeight1.get(s));
...
*/
		...
		totalTime += (endTime2-startTime2); 
			recallprint.println("time1"+totalTime);
		if(useClassifier)
		...
		int gr=0, trgr=0;
		HashMap<pair,Boolean> processed_edges = new HashMap<pair,Boolean>();
		while(true){
			...
			for(int i=curr_iter;i<SimilarityList.size();) {
				...
				int v= (int) curr[2];
				pair p = new pair(u,v);
				if(processed_edges.containsKey(p))
					continue;
				...
				if((u==715968 && v==874030) || (v==715968 && u==874030))
					System.out.println("this is something wrong??"+output[1]+" "+output[0]);		
				if(output[1]) {
			...
			recallprint.println(trgr*1.0/1155+" "+trgr*1.0/gr+" "+curr_iter+" "+gr/goldMap.keySet().size()+" "+trgr+" "+gr+" "+actualqueried_edge_map.keySet().size()+" "+actual_comparisons);	
	...
	/*
		...
		for(pair p:actualqueried_edge_map.keySet()){
			System.out.println(p.x+" "+p.y+" "+recordList1.get(p.x)+" "+recordList2.get(p.y));
		...
		for(pair p : goldMap.keySet()) {
			...
			if(!out[1]){
				...
				if(!edge_prob.containsKey(p)){
					pr.println(p.x+" "+p.y);
					pr.println(p.x+" "+p.y+" "+recordList1.get(p.x)+" "+recordList2.get(p.y));
					...
					}for(String s: inverted_list2.get(p.y)){
						pr.println(s+" "+blockWeight1.get(s)+" "+blockSize.get(s));
					})
		...
		System.out.println(blockWeight1.get("sony")+" "+blockSize.get("sony"));
	...
	*/	/*gr=0;
	...
	public static void main(String[] args) throws Exception{
		...
		CleanClassifierBlockingproducts FF = new CleanClassifierBlockingproducts();
