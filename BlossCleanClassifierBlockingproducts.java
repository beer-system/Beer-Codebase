...
public class BlossCleanClassifierBlockingproducts {
			...
			for(int i1=0;i1<lst1.size();i1++){
				...
				for(int i2=0;i2<lst2.size();i2++){
					...
					if(processed.containsKey(t))
						...
						//if(node_degree1.get(u)>100 || node_degree2.get(v)>100)
						//	continue;
...
*/		//SimilarityList.clear();	
		...
		System.out.println("going to bloss");
		//Call Bloss
		//Create levels
		//Send these to python file
		//
	...
	public void process_pairs_levels(ArrayList<ArrayList<pair>> levelList, HashMap<pair,ArrayList<Integer>> blocking_features) throws FileNotFoundException{//,Exception{
		...
		for (ArrayList<pair> level: levelList){
			...
			while(true){
				pair best = get_best_pair(covered_features,level,blocking_features);	
				if(best==null)
					break;
				else{
					training.add(best);
				...
				System.out.println(training.size()+" "+level.size()+" "+covered_features.keySet().size());
			}
		}
		int gr=0,rd=0;
		double prob = 0.0;
		...
		int higher = 0;
		/*
		ArrayList<pair> cleaned_training = new ArrayList<pair>();
		for(pair p:training){
			if(!goldMap.containsKey(p) && edge_prob.get(p)> avg)
				higher++;
			else
				cleaned_training.add(p);
		}
		...
		*/
		...
		//call learner here
		String[] args1 = new String[] {"/bin/bash", "-c", "cd ~/javaCode/src/products; python learnerbloss.py > b1; cd .."};
		try{
	...
	public double[] get_cficf_score(pair p){
		...
		ArrayList<String> list2 = inverted_list2.get(p.y);
		for (String s1: inverted_list1.get(p.x)){
			...
			if(list2.contains(s1)){
				inter++;
				reciprocal_score+=1.0/(BlockTreeMap1.get(level).get(s1).size() + BlockTreeMap2.get(level).get(s1).size());
			}
		}
		for(String s2:list2){
...
*/
	...
	*/	/*gr=0;
	...
	public static void main(String[] args) throws Exception{
		...
		BlossCleanClassifierBlockingproducts FF = new BlossCleanClassifierBlockingproducts();
