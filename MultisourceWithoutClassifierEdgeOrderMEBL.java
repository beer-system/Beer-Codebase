...
public class MultisourceWithoutClassifierEdgeOrderMEBL {
	Random rand = new Random(1991);
	String folder = "cameramulti";
	...
	public int get_partition(int u) {
		return partMap.get(u);
			...
			//		continue;	
				...
				for(int i1=0;i1<lst.size();i1++){
					...
					for(int i2=i1+1;i2<lst.size();i2++){
						...
						if(get_partition(u)==get_partition(v))
							continue;
						pair t = new pair(u,v);
						pair t1 = new pair(v,u);
...
//			System.out.println(recordList1.get(p.x)+" "+s1);
	...
	public int mergeBlocks() {
		...
		for(Integer rec_id : recordList.keySet()) {
			...
			String[] rec = recordList.get(rec_id).split(" ",0);//dmp.split(" ",0);
			int i=0;
	...
	//			System.out.println(s+" "+level_map.get(s).size());
				...
				if(level_map.get(s).size()>10 || tokenLst.length==1) {
					...
					if(s.equals("dtbvjmfvbsvhzkon2dlht"))
						System.out.println("found");	
	...
	public void processBlocks() throws Exception{
		...
		PrintStream newgt = new PrintStream("newgttmp.txt");
...
//		HashMap<String,Double> ruleBlockWeight = new HashMap<String,Double>();
		...
		*/		if(output[0])
					newgt.println(u+" "+v);
				//System.out.println(curr[0]+" "+output[1]+ " "+u+" "+v);
				if(output[1]) {
...
/*
			...
			if(processed_nodes.size()==N)
				break;
			if(processed_nodes.size()%1000000==0){
...
*/		this.recallprint.println(processed_nodes.size()+" "+this.N);
	...
	public static void main(String[] args) throws Exception{
		...
		MultisourceWithoutClassifierEdgeOrderMEBL FF = new MultisourceWithoutClassifierEdgeOrderMEBL();
		...
		String partFile     = FF.folder+"/partition.txt";

                Scanner partScanner = new Scanner(new File(partFile));
                while(partScanner.hasNextLine()){
			line= partScanner.nextLine();
			String[] parttokens = line.split(" ",0);
			int u=Integer.parseInt(parttokens[0].replace(".jpg", ""));
			int id = Integer.parseInt(parttokens[1]);
			FF.partMap.put(u,id);
		}	

		String goldFile     = FF.folder+"/newgt.txt";
		...
		while(goldScanner.hasNextLine()){
			...
			pair p =new pair(u,id);
			pair p1 =new pair(id,u);
			FF.oracle.put(p,true);
			FF.oracle.put(p1,true);
			FF.goldMap.put(u, id);
		}
